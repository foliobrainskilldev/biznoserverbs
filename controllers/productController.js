...
exports.createProduct = async (req, res) => {
    try {
        ...
        if (!await checkPlanLimits(req.user, 'image', { newImageCount: req.files?.length || 0 })) {
            return res.status(403).json({ success: false, message: `O plano permite no máximo ${plan.imageLimitPerProduct} imagens por produto.` });
        }
        ...
        if (!name || !price || !category) return res.status(400).json({ success: false, message: 'Nome, preço e categoria obrigatórios.' });

        let uploadedImages = [];
        if (req.files?.length) {
            const results = await Promise.all(req.files.map(file => cloudinary.uploader.upload(file.path, { folder: `bizno/${req.user.id}/products`, resource_type: "image" })));
        ...
        if (originalPrice && parseFloat(originalPrice) > parseFloat(price)) {
            promotionData = { originalPrice: parseFloat(originalPrice), discountPercentage: calculateDiscountPercentage(originalPrice, price) };
        }

        const product = await prisma.product.create({
            data: {
                userId: req.user.id, name, price: parseFloat(price), categoryId: category, description, stock: stock ? parseInt(stock) : 0,
                images: uploadedImages, isFeatured, promotion: promotionData
            }
        });

        res.status(201).json({ success: true, message: 'Produto criado!', product });
    } catch (error) {
...
exports.updateProduct = async (req, res) => {
    try {
        ...
        if (req.files?.length) {
            const results = await Promise.all(req.files.map(file => cloudinary.uploader.upload(file.path, { folder: `bizno/${req.user.id}/products` })));
        ...
        const updatedProduct = await prisma.product.update({
            where: { id },
            data: { name, price: parseFloat(price), categoryId: category, description, stock: stock ? parseInt(stock) : 0, images: finalImages, isFeatured, promotion: promotionData }
        });

        res.status(200).json({ success: true, message: "Produto atualizado!", product: updatedProduct });
    } catch (error) {
...
exports.deleteProduct = async (req, res) => {
    try {
        ...
        if (product.images?.length) await cloudinary.api.delete_resources(product.images.map(img => img.public_id).filter(Boolean), { resource_type: 'image' });
        if (product.video?.public_id) await cloudinary.uploader.destroy(product.video.public_id, { resource_type: 'video' });
...
exports.toggleProductFeature = async (req, res) => {
    try {
        const product = await prisma.product.findFirst({ where: { id: req.params.id, userId: req.user.id } });
        if (!product) return res.status(404).json({ success: false, message: 'Produto não encontrado.' });
        if (!product.isFeatured && !await checkPlanLimits(req.user, 'featured_product')) return res.status(403).json({ success: false, message: 'Limite atingido.' });
        
        const updatedProduct = await prisma.product.update({ where: { id: product.id }, data: { isFeatured: !product.isFeatured } });
    ...
    } catch (error) { handleError(res, error, 'Erro ao alterar destaque.'); }
};
...
exports.addProductVideo = async (req, res) => {
    try {
        const product = await prisma.product.findFirst({ where: { id: req.params.id, userId: req.user.id } });
        if (!product) return res.status(404).json({ success: false, message: 'Produto não encontrado.' });
        if (!await checkPlanLimits(req.user, 'video', { isExistingVideo: !!product.video })) return res.status(403).json({ success: false, message: 'Limite de vídeos atingido.' });
        ...
        const updatedProduct = await prisma.product.update({ where: { id: product.id }, data: { video: { url: result.secure_url, public_id: result.public_id } } });
        res.status(200).json({ success: true, message: 'Vídeo adicionado.', product: updatedProduct });
    } catch (error) { handleError(res, error, 'Erro ao adicionar vídeo.'); }
};

exports.getProducts = async (req, res) => {
    try {
        const products = await prisma.product.findMany({ where: { userId: req.user.id }, include: { category: { select: { name: true } } }, orderBy: { createdAt: 'desc' } });
...
exports.createCategory = async (req, res) => {
    try {
        if (!req.body.name) return res.status(400).json({ success: false, message: 'O nome é obrigatório.' });
        const category = await prisma.category.create({ data: { userId: req.user.id, name: req.body.name } });
        res.status(201).json({ success: true, message: 'Categoria criada.', category });
    } catch (error) { handleError(res, error, 'Erro ao criar categoria.'); }
};
...
exports.getCategories = async (req, res) => {
    ...
    } catch (error) { handleError(res, error, 'Erro ao buscar categorias.'); }
};
...
exports.updateCategory = async (req, res) => {
     try {
        ...
        const updatedCategory = await prisma.category.update({ where: { id: category.id }, data: { name: req.body.name } });
        res.status(200).json({ success: true, message: 'Categoria atualizada.', category: updatedCategory });
    } catch (error) { handleError(res, error, 'Erro ao atualizar categoria.'); }
};
...
exports.deleteCategory = async (req, res) => {
    try {
        ...
        if (productCount > 0) return res.status(400).json({ success: false, message: `Usada por ${productCount} produto(s). Não pode ser removida.` });
        
        await prisma.category.delete({ where: { id: category.id } });
        res.status(200).json({ success: true, message: 'Categoria removida.' });
    } catch (error) { handleError(res, error, 'Erro ao remover categoria.'); }
};